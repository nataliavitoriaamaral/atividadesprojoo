...
public class Program
{
    ...
    {
        // Solicita a criação do SMS Externo através da Factory
        INotificacao smsAdaptado = ComunicacaoFactory.CriarComunicacao("SMS_externo");
        smsAdaptado.Enviar("Alerta de sistema via API externa.");
    }

    public static void TesteProxy()
    {
        INotificacao emailReal = ComunicacaoFactory.CriarComunicacao("EMAIL");
        INotificacao proxySeguro = new ProxyNotificacaoSegura(emailReal);

        // Testa o bloqueio de mensagem vazia
        proxySeguro.Enviar("");

        // O limite do Singleton é 5, então tentamos enviar 6 vezes para ver se ocorre bloqueio 
        for (int i = 1; i <= 6; i++)
        {
            proxySeguro.Enviar($"Tentativa de envio número {i}");
        }
    }
}